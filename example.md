...
And our state is an `Int(Integer)` with the count value. We start our state with the counter set to `0` and our `eval` function handle the state updation based on our actions.
